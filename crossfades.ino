...
* 0-255 values, and there are 256 steps.
* 
...
int calculateVal(int step, int val, int i, int limit) {

  if ((step) && i % step == 0) { // If step is non-zero and its time to change a value,
    if (step > 0 && val < limit) {              //   increment the value if step is positive...
      val += 1;           
    } 
    else if (step < 0 && val > limit) {         //   ...or decrement it if step is negative
      val -= 1;
    } 
  }
...
*  0-255 range, then loops 256 times, checking to see if  
...
void crossFade(int color[3]) {
  ...
  for (int i = 0; i <= 255; i++) {
    ...
    redVal = calculateVal(stepR, redVal, i, R);
    grnVal = calculateVal(stepG, grnVal, i, G);
    bluVal = calculateVal(stepB, bluVal, i, B);
...
{
    wait[activeSlot] = args.toInt();
   return wait[activeSlot];
}
...
int colorMod(String args)
{
    ...
    int newWait = starts[2].toInt();
    if(newWait > 0)
    {
        wait[activeSlot] = newWait;
    }
